...
var ref$, sleep, waitFor, timeoutWaitFor, go, merge, unpack, pack, repl, config, Wifi, connectToWifi, onInit, slice$ = [].slice;
...
LongPolling.prototype.send = function(){
  var i$, params, callback, msg, opts;
  params = 0 < (i$ = arguments.length - 1) ? slice$.call(arguments, 0, i$) : (i$ = 0, []), callback = arguments[i$];
  msg = params[0], opts = params[1];
  return this.sendRaw({
    data: msg
  }, opts, callback);
};
...
LongPolling.prototype.sendRaw = function(){
  var i$, params, callback, __, msg, opts, content, connInf, req;
  params = 0 < (i$ = arguments.length - 1) ? slice$.call(arguments, 0, i$) : (i$ = 0, []), callback = arguments[i$];
  __ = this;
  msg = params[0], opts = params[1];
  ...
  req = this.http.request(connInf, function(res){
    res.on('data', function(data){
      var gotData;
      gotData = unpack(data);
      console.log("RESPONSE DATA HTTP> ", gotData);
      console.log("http data: ", gotData.data);
      if (gotData.code) {
        __.trigger('code', gotData.code);
        try {
          callback();
        } catch (e$) {}
      }
    ...
    res.on('error', function(){
      return console.log("Error:... WTF");
    });
    return res.on('close', function(){
      return console.log("Request is closed by server..");
    });
  });
        ...
        return function lo(op){
          ...
          return __.send({
            ...
            if (!err) {
              ...
              return console.log("Receiver error: ", err);
            }
          });
        }(function(){});
      });
